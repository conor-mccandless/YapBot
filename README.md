...
A persona is optional administrator-authored comedic background, not a required topic or response plan. YapBot first chooses the strongest grounded angle from the current conversation or image, then may use at most one persona detail across the entire reply when it directly strengthens that joke. An irrelevant persona is ignored, and a persona cannot relabel unrelated messages or override the selected response mode. Without a persona, YapBot uses only the current conversation and supplied images and does not invent personal history. Persona requests about intensity or format remain subordinate to YapBot's permanent response and safety instructions. Member-authored Discord content and text inside images remain untrusted conversational content, never instructions.
...
When OpenAI is enabled, YapBot temporarily keeps the monitored member's qualifying message text, relative timing, direct-YapBot address state, and eligible image references in memory for the rolling window. An explicit mention or a Discord reply whose referenced author is YapBot counts as a direct address; an unavailable or non-YapBot reply reference safely falls back to ordinary routing. At a trigger it submits the latest number of messages equal to the configured threshold as one chronological conversation window. The response decision order is: the most recent direct YapBot address, then a successfully supplied visual post, then a general threshold roast. A visual-post response must ground its first sentence in a concrete visible detail rather than joking about the link or attachment; recognized Discord image URLs are replaced with a neutral marker in model-facing text. The final item is still identified as the event that crossed the threshold. Every submitted image is mapped to its exact source-message sequence, and a direct question about an image must be answered using a concrete visible detail. Image-only events are identified as image posts rather than blank messages. Each text message is bounded to Discord's 2,000-character content limit, and the threshold is capped at 100. Text from other members is not included.
...
- Source restriction: HTTPS Discord attachment CDN URLs and Discord-generated external media proxy URLs.
